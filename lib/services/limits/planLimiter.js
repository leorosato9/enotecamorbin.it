...
export async function checkRestaurantLimit(db, userId, userPlan) {
  ...
  if (count >= limit) {
    // Messaggio di errore come semplice testo
    let message = `Hai raggiunto il limite di ${limit} ristoranti per il piano ${PLAN_CONFIG[userPlan].name}. Fai l'upgrade a Plus per aggiungerne altri!`;
...
export async function getMenuPerWeekStatus(db, userId, userPlan) {
    ...
    if (count >= limit) {
        // Messaggio di errore come semplice testo
        let message = `Hai raggiunto il limite di ${limit} carta dei vini a settimana per il piano ${PLAN_CONFIG[userPlan].name}. Fai l'upgrade o attendi per crearne di nuove!`;
...
export async function checkMenuPerWeekLimit(db, userId, userPlan) {
  ...
  if (!status.allowed) {
    // Lanciamo un errore con solo il messaggio di testo
