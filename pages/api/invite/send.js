...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Controlla che l’attività esista e che l’utente sia proprietario
      ...
      if (activity.userEmail !== inviterEmail) {
        return res.status(403).json({ message: 'Non autorizzato a invitare' });
      }

      // Inserisci l’invito nella collezione "inviti"
      const nuovoInvito = {
...
Sei stato invitato da ${inviterEmail} a collaborare sull’attività "${activity.nome}".
...
Enoteca Morbin
        ...
        <div>
          ...
          <p><strong>Da parte di:</strong> ${inviterEmail}</p>
  ...
  if (req.method === 'GET') {
    // Recupera inviti pendenti per l’utente loggato
    const session = await getServerSession(req, res, authOptions);
    if (!session) {
      return res.status(401).json({ message: 'Non autenticato' });
    }
    ...
    try {
      ...
      const pendingInviti = await db
        .collection('inviti')
        .find({ inviteeEmail, status: 'pending' })
        .sort({ createdAt: -1 })
        .toArray();
  ...
  // Se non POST né GET, rispondi metodo non consentito
